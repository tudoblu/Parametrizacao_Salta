...
if df_juntas.empty and df_veios.empty:
    ...
    afloramento_selecionado = st.selectbox(
        ...
        index=afloramentos_opcoes.index('VINUALES')  # Define 'VINUALES' como padrão
    )
    ...
    if afloramento_selecionado == 'Todos':
        ...
        camadas_do_afloramento.extend(
            df_veios[df_veios['Afloramento'] == afloramento_selecionado]['Camada'].dropna().unique().tolist()
        )
